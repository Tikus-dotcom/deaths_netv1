...
class ProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_CONNECT(self):
        if sets.RHTTPS:
            ...
            try:
                if self.connect_to(soc, self.host, self.port, self.advhost):
                    data = '%s 200 Connection Established' % (self.request_version)
    ...
    def get_recv_headers(self):
        self.send_connection_logger(P+'++++++++++++ REQUEST! ++++++++++++\r\n')
        self.send_connection_logger(W+'[+] From Adress - %s:%s\r\n[+] %s\r\n' % (self.client_address[0], self.client_address[1], self.requestline))
        for header, value in self.headers.items():
            self.send_connection_logger(C+'[+] %s : Locked\r\n' % str(header).title())
        self.send_connection_logger('\r\n')
    def get_send_inject(self):
        if sets.IQUERY:
            self.send_connection_logger(B+'++++++++++ SEND INJECT! ++++++++++\r\n')
            if self.phost and self.pport:
                self.send_connection_logger(C+'[+] Using Proxy : Locked\r\n')
            elif sets.ADMODE:
                self.send_connection_logger(W+'[+] Using Host - %s:%s\r\n' % (self.advhost, self.port))
            else:
                self.send_connection_logger(W+'[+] Using Server - %s:%s\r\n' % (self.host, self.port))
            for inject in self.splitline[0].split(self.newline):
                self.send_connection_logger('%s' % inject)
            self.send_connection_logger(C+'[+] Host : Locked\r\n\r\n')
    ...
    def send_connection_error(self, msg, page = True):
        ...
        self.send_connection_logger(R+'[+] Connection Error : Locked\r\n')
